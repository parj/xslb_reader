...
class RecordReader:
    ...
    def __next__(self) -> Tuple[int, bytes]:
        if self._pos >= len(self._buf):
            raise StopIteration
        # record type
        b0 = self._buf[self._pos]
        self._pos += 1
        if b0 & 0x80:
            if self._pos >= len(self._buf):
                raise StopIteration
            b1 = self._buf[self._pos]
            self._pos += 1
...
def _load_ftab_from_local_spec() -> Dict[int, str]:
    ...
    section = (
        text[m_start.end() : m_start.end() + m_end.start()]
        if m_end
        else text[m_start.end() :]
    )
        ...
        while self._buf.tell() < self._end:
            ...
            if ptg < 0x20:
                ...
                if ptg in _BINOP:
                    ...
                    row = self._u32()
                    col = (
                        struct.unpack("<I", self._rgcb[:4])[0]
                        if len(self._rgcb) >= 4
                        else 0
                    )
            ...
            # ================================================================
                ...
                if base == _B_ARRAY:
                    ...
                    nameindex = self._u32()
                    stack.append(self._dnames.get(nameindex, f"name{nameindex}"))
                ...
                elif base in (_B_MEM_AREA, _B_MEM_ERR, _B_MEM_NOMEM, _B_MEM_FUNC):
                    self._buf.read(4)
                    self._u16()  # 4 reserved + cce
...
def _read_sst(data: bytes) -> List[str]:
    ...
    for rec_type, payload in RecordReader(data):
        if rec_type == BRT_SST_ITEM:
            ...
            strings.append(buf.read(cch * 2).decode("utf-16-le", errors="replace"))
    return strings
...
def _parse_worksheet(
    ...
    for rec_type, payload in RecordReader(data):
        if rec_type == BRT_ROW_HDR:
            ...
            buf = io.BytesIO(payload)
            try:
                rw_first, rw_last, col_first, col_last = struct.unpack(
                    "<IIII", buf.read(16)
                )
            except struct.error:
                continue
...
class XlsbWorkbook:
    ...
    def iter_formulas(
        ...
        for sheet_name, rel_id in self._sheets:
            ...
            yield (
                sheet_name,
                _parse_worksheet(ws_data, all_names, self._sst, self._dnames),
            )
